...
export default function LoginPage() {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-zinc-950 p-4 overflow-hidden relative">
      {/* Background decorations */}
      <div className="absolute top-0 left-0 w-full h-[500px] bg-gradient-to-b from-indigo-500/20 via-violet-500/10 to-transparent pointer-events-none" />
        ...
        <CardContent className="px-8 pb-8">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              <label htmlFor="email" className="text-sm font-semibold text-white/90 tracking-wide">
                Correo Electrónico
              </label>
              <Input
                id="email"
                ...
                className="bg-white/95 border-0 mt-2 focus-visible:ring-2 focus-visible:ring-violet-500 text-zinc-950 placeholder:text-zinc-400 font-medium h-11 shadow-inner"
                required
              />
            </div>
            ...
            <Button 
              type="submit" 
              className="w-full h-11 text-base font-semibold bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 border-0 shadow-lg shadow-violet-500/25 transition-all mt-6" 
              disabled={loading}
            >
              {loading ? (
